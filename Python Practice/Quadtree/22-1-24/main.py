...
'''
Last Code Update Date: 22/01/2024
...
# Local Variables
img_path = '/workspaces/Practice-2024/Python Practice/Quadtree/19-1-24/img/'
quadree_sturcture_csv = '/workspaces/Practice-2024/Python Practice/Quadtree/19-1-24/data/'
DPI = 100 # Dots per inch.
capacity = 4 # Data points capacity to each rectangle in quadtree.
...
df = data[['CMPLNT_FR_DT','CMPLNT_FR_TM','Latitude','Longitude']].head(12)
# print(df.info())
...
# We are selecting appropriate width and height based on the data.
...
qtree = QuadTree(domain, capacity, rectangle_id=1)    # create or initialise a quadtree with passing the arguments rectangle and capacity.
...
for point in points:
    label = rectangle_association.get(point, None)
    if qtree.insert(point, label=label):
        rectangle_association[point] = qtree.rectangle_id

# # Recursively store in csv file.
# qtree.save_to_csv(quadree_sturcture_csv + 'quadtree_info.csv')
# qtree.organize_data_points()
